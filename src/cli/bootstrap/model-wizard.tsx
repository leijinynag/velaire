...
function buildWizardModelEntry(baseURL: string, apiKey: string, modelName: string, provider: ModelEntry["provider"]): ModelEntry {
  return {
    name: modelName.trim(),
    provider,
...
function ModelWizard({ onComplete, onAbort }: ModelWizardProps) {
  ...
  useInput((_input, key) => {
    if (step === "provider") {
      if (key.upArrow) setProviderIndex((index) => (index > 0 ? index - 1 : MODEL_PROVIDERS.length - 1));
      if (key.downArrow) setProviderIndex((index) => (index < MODEL_PROVIDERS.length - 1 ? index + 1 : 0));
  ...
  const finishWithBaseURL = (url: string) => {
    ...
    const entry = buildWizardModelEntry(url, apiKey, modelName, selectedProvider.providerType);
  ...
  const goFromModelName = () => {
    if (!selectedProvider.baseURL) {
      ...
      const entry = buildWizardModelEntry(selectedProvider.baseURL, apiKey, modelName, selectedProvider.providerType);
      setPendingEntry(entry);
      setStep("confirm");
    }
  };
  ...
  useInput((input, key) => {
    ...
    if (key.return) {
      if (pendingEntry) onComplete(pendingEntry);
      return;
    }
    ...
    return (
      <Box flexDirection="column">
        ...
        {MODEL_PROVIDERS.map((provider, index) => (
          <Text key={provider.id} color={index === providerIndex ? "cyan" : undefined}>
            {index === providerIndex ? "❯ " : "  "}
            {provider.label}
          </Text>
        ))}
  ...
  if (step === "modelName") {
    return (
      <Box flexDirection="column" rowGap={1}>
        ...
        <Box>
          <Text>Model: </Text>
          <TextInput value={modelName} placeholder="e.g. claude-sonnet-4-6" onChange={setModelName} onSubmit={goFromModelName} />
        </Box>
        <Text color={currentTheme.colors.dimText}>Press Enter to continue (Esc to cancel)</Text>
      </Box>
    );
  ...
  if (step === "confirm") {
    ...
    return (
      <Box flexDirection="column" rowGap={1}>
        <Text bold color="cyan">Last confirmation</Text>
        <Text>Provider: {selectedProvider.label}</Text>
        <Text>Model: {entry.model}</Text>
        <Text>baseURL: {entry.baseURL ?? "(provider default)"}</Text>
  ...
  return (
    <Box flexDirection="column" rowGap={1}>
      ...
      <Box>
        <Text>baseURL: </Text>
        <TextInput value={customBaseURL} onChange={setCustomBaseURL} onSubmit={() => finishWithBaseURL(customBaseURL)} />
      </Box>
...
function maskSecurityString(value: string): string {
  try {
    return value.slice(-8).padStart(value.length, "*");
  } catch {
